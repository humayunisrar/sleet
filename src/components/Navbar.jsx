...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar relative z-20">
      <img src={logo} alt="logo" className="xs:w-[180px] xs:h-auto w-[120px] h-auto" />

      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`font-inter font-normal cursor-pointer text-[15px] ${active === nav.title ? "text-white" : "text-dimWhite"
        ...
        >
          <ul className="list-none flex justify-end items-start flex-1 flex-col">
            {navLinks.map((nav, index) => (
              <li
                key={nav.id}
                className={`font-inter font-medium cursor-pointer text-[16px] ${active === nav.title ? "text-white" : "text-dimWhite"
